...
var redips = {};
var initDisplay, displayLayout, scanTable, scanObject, injectObject, editLayout, editButton,editTable, makecontextmenu, test
, initEdit,editMenu, editObject, editcell, editcellsave, tableResize, reSize,delRow,delCol, prefixFind, prefixFind1, showContent, getContent;
var menuid = "", subid = ""; //현재 선택한 menu, submenuid
// configuration
redips.configuration = function (opt) {
    ...
    if (typeof opt != "undefined") {
        if (opt.hasOwnProperty("component")) redips.components = opt.components;
        if(opt.hasOwnProperty("tableEditor"))redips.tableEditor = opt.tableEditor;
        if (opt.hasOwnProperty("layout")) redips.layout = opt.layout;
    }
};
// initialization
redips.init = function (opt) {
    // define reference to the REDIPS.drag and REDIPS.table object
    var rt = REDIPS.table,
		rd = REDIPS.drag;
    // configuration
    redips.configuration(opt);
...
initDisplay = function (id, data,insertdv) {
    ...
    var dv;
    if ($("#" + redips.layout).length == 0)
        $(" <div id='"+redips.layout+"' style='margin-top:10px' class='layout' ></div><div style='clear:both;margin-bottom:20px;'/>").appendTo($($("form")[0]));
...
injectObject = function (dvid) {
    if (typeof dvid == "undefined") dvid = $("#" + redips.layout);
    var list = ["jqgrid", "select", "jstree", "googlechart", "fullcalendar", "content", "map", "ifrm", "pivot","rstat", "form","otherpage","html"];
    //if imcdata not exists
    if (localStorage.getItem("imcdata") == null)
        dataListOnlyAjax();
...
initEdit = function () {
    if ($("#" + redips.layout).length == 0)
        $(" <div id='" + redips.layout + "' style='margin-top:10px' class='layout' ></div>").appendTo($($("form")[0]));
    $(".layout").empty();
    $(".layout").append(editLayout());
    $("#" + redips.tableEditor).colResizable({ disable: true });
    tableResize($("#"+redips.tableEditor));
    if (redips.layout == "tableinsert")
        editMenu();
    funStop();
}
editMenu = function () {
    ...
    $("#" + redips.layout).css({ "margin-top": "10px" });
...
editLayout = function () {
    ...
    var list = [["inE", "Apply", "funLoading(true);editSave(true); initDisplay('', selectimctable(menuid, subid)); menuInit();"]
   ...
   // table1.attr("id", "tblToolbox");
    ...
    var list = //[["Merge", "reSize('dis');redips.merge();reSize('en')"]
        ...
        //, ["Split V", "reSize('dis');REDIPS.table.split('v');reSize('en')"]
         [["R+", "reSize('dis');REDIPS.table.row('" + redips.tableEditor + "','insert');reSize('en')"], ["R-", "reSize('dis');delRow();reSize('en')"]
        , ["C+", "reSize('dis');REDIPS.table.column('" + redips.tableEditor + "','insert');reSize('en')"], ["C-", "reSize('dis');delCol('" + redips.tableEditor + "');reSize('en')"]];
    $.each(list, function (i, k) {
        var inp = $("<input/>");
        var td1 = $("<td style='padding:0 0 5px 0;'/>");
        td1.attr("class", "mark");
        inp.attr("type", "button");
...
editTable = function (option) {
    var wth="200px,200px,200px",data="";
    if(typeof option !="undefined"){
       
        if (option.hasOwnProperty("width")) wth = option.width;
        if (option.hasOwnProperty("data")) data = option.data;
    }
    //right-middle table
    var table1 = $("<table/>")
    table1.attr("id", redips.tableEditor);
    ...
    if (data != "") {
        ...
        $.each(data, function (rowIndex, r) {
            ...
            $.each(r, function (colIndex, c) {
                ...
                var dv = $("<div/>");
                dv.attr("style", "text-align:center;height:22px;");
...
delRow = function () {
    ...
    if (emptyrow)
        REDIPS.table.row(redips.tableEditor, 'delete');
    else
        swal("Last row is not empty!!");
}
delCol = function () {
    //check last row,col has controls?
    var lasttds = $("#" + redips.tableEditor + ":first tr td:last-child");
    var empty = true;
    ...
    if (empty)
        REDIPS.table.column(redips.tableEditor, 'delete');
    else
        swal("Last column is not empty!!");
}
var wtharr = [];
reSize = function (type) {
    var id=redips.tableEditor;
    var tb = $("#"+id);
    tb.attr("class", "ui-widget");
    switch (type) {
        case "dis":
            wtharr = [];
            $("#"+id+" th").each(function (i, k) {
        ...
        case "en":
            ...
            //th insert
            var row = $("#" + id + " tr:eq(0)").children();
            var thead = $("<thead class='ui-widget-header'/>").prependTo("#"+redips.tableEditor);
...
scanTable = function () {
    ...
    $("#" + redips.tableEditor).find('tr').each(function (i, tr) {
        trr = [];
...
function editSave(remote) {
    ...
    var set = {};
    var width = [];
    $("#"+redips.tableEditor+" tr:eq(0) th").each(function () {
...
editcellsave = function (cellid) {
    ...
    var cell = $("#" + redips.tableEditor).find("tr:eq(" + row + ")").find("td:eq(" + rc[1] + ")");
    cell.attr("style", rdt);
    ...
    rowSortable($("#" + redips.tableEditor));
