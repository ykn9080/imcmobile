...
function iffileup(rtnurl) {
   ...
   // $("#dialog").remove();
    //var dia=$("<div id='dialog' style='z-index:200;'/>");
    //dia.append($("<iframe style='width:350px;height:250px;' frameborder='0' scrollbars=no id='ifUpload' />"));
   //window.open(url, "정보공유설정, status = 1, height =250px, width =350px, left=300px,top=300px,resizable = 1");

    //var rtn = $("#dveditback");//$("#" + rtnurl).parent();//.parent().parent();//.parent().parent();//.parent().parent();//.parent().parent();
    //rtn.append(dia);
  //  $("#dialog").dialog({
  //    autoOpen: true,
  //    show: "fade",
...
function actionclick(val, datacode, dndcontain, ctrtype) {
    ...
    $(Object.keys(sessionStorage)).each(function (i, k) {
    ...
    function locallist(local) {
        var dv = $("<div />");
        var localarr = Object.keys(sessionStorage);
        var ul = $("<select id='sellocal'/>"), li = $("<li/>"), grp = $("<optgroup/>"), selected = "";
        li = $("<option  value='' disabled selected>Select sessionStorage</option>");
        ul.append(li);
        $(localarr).each(function (i, k) {
            ...
            if ($.inArray(k, ["formlist", "imcdata"]) == -1) {
                ...
                grp.attr("label", k);
                ul.append(grp);
                var kk = JSON.parse(sessionStorage.getItem(k));
        ...
        var btn = $("<i style='margin-left:5px' title='add new sessionStorage'  id='btnnewlocal' class='fa fa-plus-square imdim'/><span id='btnnewlocal1' class='linkbtn' title='add new sessionStorage'>add</span>");
...
function trstyleInsert(option) {
    ...
    if (typeof option != "undefined") {
        ...
        if (option.hasOwnProperty("ctrid")) {
            switch (option.type) {
                case "formlist":
                    var flist = sessionStorage.getItem("formlist");
                    if (flist != "") {
